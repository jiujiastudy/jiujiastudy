...
一整天还挂着「今天」。这里钉住：过没过期看真实时刻；今天 / 明天 / 还有几天按显示出来的那个日期数，
两边才对得上；--date 指的是一个时刻。

2026-09-23 起（发起人：「时间以用户所在地为主，课程那边只标一次」）显示的是用户那边的日期和时刻，
所以天数也按用户那边的日历数（两地时区相同时和以前一样）。下面的期望值按这条规则逐条手算过。
...
CASES = [
    ...
    ("上海的课 · 人在伦敦 · 课程那边已经是第二天", "Asia/Shanghai", "Europe/London",
     "2026-09-20T17:00:00Z", "2026-09-25T15:59:00Z", "还有 5 天", 5, False),  # 伦敦 09-20 18:00 → 09-25 16:59
    ...
    ("美东的课 · 人在上海 · 过了半小时", "America/New_York", "Asia/Shanghai",
     "2026-09-25T04:30:00Z", "2026-09-25T03:59:00Z", "已过期", 0, True),  # 上海同一天 11:59 截止，12:30 已过
    ...
    ("悉尼的课 · 人在美西 · 明天", "Australia/Sydney", "America/Los_Angeles",
     "2026-10-04T14:00:00Z", "2026-10-06T12:59:00Z", "还有 2 天", 2, False),  # 洛杉矶 10-04 07:00 → 10-06 05:59
    ("悉尼换夏令时那天 · 今天", "Australia/Sydney", "Asia/Shanghai",
     "2026-10-03T15:30:00Z", "2026-10-04T12:59:00Z", "明天", 1, False),  # 上海 10-03 23:30 → 10-04 20:59
    ("悉尼换夏令时那天 · 明天", "Australia/Sydney", "Asia/Shanghai",
     "2026-10-03T15:30:00Z", "2026-10-05T12:59:00Z", "还有 2 天", 2, False),  # 上海 10-03 23:30 → 10-05 20:59
    ...
    def test_两地日期不一致时每一行怎么算(self):
        for label, course_tz, user_tz, now, due, rel, days_left, overdue in CASES:
            with self.subTest(case=label):
                ...
                self.assertEqual(clock.show_date(cc_time.parse_ts(due)), r["date"], "分天用的日期和显示的是同一个")

    def test_只写用户那边的时间_不解释时区(self):
        # 人在上海：每行只写上海的日期和时刻，不再并列两个；页眉也不解释时区（发起人：时间只要知道是什么时候出的）。
        clock, rows = self.rows("America/New_York", "Asia/Shanghai", "2026-09-25T00:59:00Z", "2026-09-25T03:59:00Z")
        self.assertEqual("09-25 周五 11:59", rows[0]["when"].split("，")[0])
        self.assertNotIn("美东", rows[0]["when"])
        self.assertEqual("", clock.tz_note(clock.today_user()))
