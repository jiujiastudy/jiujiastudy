...
| 「这周学什么」「这周干什么」 | `collect --touch` → `study --write`；要润色就把中文写进 `--zh` 覆盖层再跑一次；复述本周最要紧的一件和今天的必做 | study.md |
...
| 情况 | 做 |
|---|---|
| 没有可调用的 Python | 先用宿主公开的 Python（Codex：`load_workspace_dependencies`）并查 `python3` / `python` / Windows `py -3` 和已安装路径；确认全部不可用后，先征得用户同意再按平台安装。不要把 `xcode-select --install` 当 Python 安装器 |
| 缺 token | `doctor --env-dialog`，一句话说清去哪生成、粘进哪个窗口，不用重启 |
| token 贴进了对话 | 不写进任何命令或文件；弹窗口让用户粘，只说「粘进刚弹出的窗口就行」 |
| 401 | 「token 多半复制不全，重新生成再粘一次」，不换学校 |
| 认不出学校 / 有几个候选 | 让用户发登录页网址或选一个，`doctor --host 网址` |
| 连不上 / 5xx | 脚本已重试；用上次快照回答，标「数据截至 X」 |
| 浏览器记录读不了 / 被沙盒隐藏 | 按宿主机制申请一次只读浏览器记录权限；仍读不了就直接要网址。探测只取域名和访问次数，不读别的 |
| 周次算不出 | 问一句「这周是第 N 周」，`config set term.week1_monday <那周周一>` |
| 资料夹建不了（桌面只读、OneDrive 锁住） | `config set root <别的路径>` 再 `doctor` |
| 课件一直没下下来 | `collect --download`（前台，一次 10 个）；日志在机器档案 logs/download.log |
| 电脑时区和课程时区不同 | 正常：两地时间不同时 deadline 会并列写两个；用户人就在学校城市却不同 → 一句「把电脑时区改成 X」 |
| state / config 坏了 | 备份成 .bak，`doctor` 重建，告诉用户可能少了什么 |
| 缺 Python 库 | `doctor` 会自动 pip；失败就把它打印的命令给用户 |
| Claude Code 权限没装 | `doctor --fix-perms`；被拦就把它打印的 permissions 块给用户粘进 ~/.claude/settings.json |
