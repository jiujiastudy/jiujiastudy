...
- **要用能在你电脑上运行命令的 AI：Claude Code 或 Codex。** ChatGPT 网页版、豆包这类只能聊天的网页跑不了救驾。
...
**先改一个设置，能省掉你很多次点「允许」**：输入框左下角的权限设置，Claude Code 选「Auto」，Codex 选「帮我批准」。改完它会自己判断哪些操作安全，只有要紧的才来问你。偶尔某一步被自动模式拦下（比如「刚下载的代码」），AI 会请你临时换成「每次询问」、回一句「继续」，跑完再换回来。救驾不会替你改任何权限设置。
...
更新：跟 AI 说「更新救驾」，或者自己在 `skills/jiujiastudy` 里执行

```bash
git pull
```

要先备份旧版的话，备份放到 skills 文件夹**外面**（比如「文档」里）。放在 skills 里面，AI 工具会把它当成第二个技能加载，可能用到旧版。
...
1. 用你电脑上已有的 Python。
2. 从浏览器记录里认出你学校的 Canvas 网址（**不会带 token 去试**），让你确认是哪个学校。认不出的话，把你平时交作业的那个网站随便哪一页的网址发给它就行，是 Canvas 还是 Moodle 它自己认。
...
4. 采集，生成桌面文件夹、本周清单和 Deadline 雷达，并直接帮你打开本周清单；桌面上会多一个「本周清单」，以后双击它就能看。
...
平时这样说：「现在什么情况」「最近要交什么」「这周学什么」「做完了」「没状态」，或者直接说你想做的事：导读课件、做复习包、写东西、发帖、交作业。桌面「救驾」文件夹里有一页「我能帮你做什么」，点一句话就能复制来问 AI。

**不知道怎么办，直接问 AI**，用你自己的话说就行。
