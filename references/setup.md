...
## 顺序
0. 开始前先说一段（原文，AI 只提醒，不替用户改设置）：「开始前先改一个设置，能省掉你很多次点『允许』：看输入框左下角的权限设置，Claude Code 选『Auto』，Codex 选『帮我批准』。改完它会自己判断哪些操作安全，只有要紧的才来问你。接下来我会先认出你的学校，再请你登录或给一个 token，然后读你的课程、做好这周的清单，大概一两分钟。中途有不懂的直接问我。」之后每一步动手前说一句「现在做什么、大概多久」，做完说一句「好了，下一步是…」。
...
3. 看输出决定。**一次只问一件事：先把学校网址确认下来，确认是 Canvas 才提 token；是 Moodle 根本不提 token。**
   - 打印了「config.json：已新建」（token 早就在，站点也认出来了）→ 不用问任何事，直接第 5 步。
   - 认出一个：「你学校的 Canvas 是不是 canvas.xxx.edu？是就回我一句。」（探测只是线索，token 只发给用户确认过的那一个地址。）他说是 → 再发 token 那段：「好。现在要一个 token，相当于一把只给我用的钥匙：Canvas → Account → Settings → Approved Integrations → New Access Token，Purpose 填 救驾，Expires 设学期最后一天，把生成的一长串整段复制，直接发给我，我来存好。」
   - 有几个：「浏览器记录里有两个像 Canvas 的站点：A（常去）和 B，你学校用的是哪个？回一个就行。」选好再发 token 那段。
   - 已读到记录但没找到、或者没法读：「把你平时交作业的那个网站随便哪一页的网址发我（浏览器地址栏里 https:// 开头的那一串）。是 Canvas 还是 Moodle 不用你分，我来认。」拿到网址跑 `doctor --host 网址`：认出 Moodle → 第 3½ 步；认出 Canvas → 发 token 那段。
   ...
   3½. 要登录时（Moodle，或学校不让生成 token），弹窗前先说：「马上会弹出一个浏览器窗口（被挡住了就在任务栏找），在里面登录你学校的平台，账号密码和验证码都是你自己输，我看不到；登好它会自己关，这是正常的。登好了回我一句。」然后 `login`，照它打印的往下做。它报这里弹不出窗口、或用户说没看到窗口：先看任务栏；还没有就按宿主的办法申请在沙盒外重跑 `login`。不要改用宿主自带的浏览器让他登录：救驾读不到那里的登录，他会白登一次。
...
5. `collect --touch`（只记元数据；失败尝试不进入十分钟缓存）→ `radar --write` → `study --write --open`（写完直接用浏览器打开本周清单；资料夹在桌面上就再放一个「本周清单」快捷方式，并写好「我能帮你做什么」那一页）→ `collect --download --background`（只消费已有队列；同一档案最多一个后台 worker，命令立刻返回），然后一条消息（状态一句放最后）：「连上了：域名（浏览器记录里认出来的，不对就告诉我），N 门课。最急的一条：…（还有 N 天），第一步：…。这周最要紧的一件：…。本周清单已经帮你打开了；以后双击桌面上的『本周清单』就能看。做完一项就在上面打勾，再回来跟我说『做完了』（点页面底部的『复制，发给 AI』粘过来最快），不说我不知道你做完了。Deadline 雷达、每门课的课件和我做的东西都在桌面的『救驾』文件夹里，课件正在后台下；那个文件夹里还有一页『我能帮你做什么』，点一句话就能复制来问我。接下来你可以直接说：『这周学什么』（细看这周每天做什么）、『最近要交什么』、『做完了』、『帮我导读这周的课件』。不知道怎么办，直接问我。状态：…。建议：…。」
...
6. doctor 打印的「你需要做的事」：`--fix-perms` 这类自己能做的做掉；时区的事不用跟用户说。
...
- 时区：自动认，时间都显示成用户电脑上的时间；不用跟用户解释时区。用户说「时间按上海显示」→ `config set user_tz Asia/Shanghai`；`config set user_tz auto` 恢复跟着电脑。
